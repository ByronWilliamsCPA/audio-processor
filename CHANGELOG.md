...
- feat(security): optional API-key authentication (`SecretStr` keys, constant-time comparison) and per-client rate limiting on `/api/v1` (both default-off); streaming uploads with a hard size cap enforced on bytes read and guaranteed temp-file cleanup; fail-fast config validators (`auth_required` requires keys; `enqueue_enabled` requires the Redis backend)

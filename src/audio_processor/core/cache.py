...
def cache_invalidate(
    ...
    Returns:
        Callable[[Callable[..., Awaitable[T]]], Callable[..., Awaitable[T]]]: Decorator function that wraps async functions to invalidate cache.
        ...
        async def wrapper(*args: object, **kwargs: object) -> T:
            ...
            try:
                ...
                logger.warning(
                    ...
                    pattern=key_pattern,  # type: ignore[call-arg]
...
async def get_cached(key: str, default: CacheValue = None) -> CacheValue:
    ...
    Args:
        key (str): Cache key
        default (CacheValue): Default value if key not found

    Returns:
        CacheValue: Cached value or default
    """
    try:
        redis = await get_redis()
        # Redis library returns unknown types - safe to ignore since we control serialization
...
async def set_cached(key: str, value: CacheValue, ttl: int = 3600) -> bool:
    ...
    Args:
        key (str): Cache key
        value (CacheValue): Value to cache
...
async def warm_cache(
    ...
    Args:
        key (str): Cache key
        value_fn (Callable[[], Awaitable[CacheValue]]): Async function to get the value
        ttl (int): Time to live in seconds
...
from audio_processor.core.cache import get_redis, close_redis
...
from audio_processor.core.cache import cached
...
from audio_processor.core.cache import invalidate_pattern
...
async def get_cache_stats() -> dict[str, int | float | str]:
    ...
    Returns:
        dict[str, int | float | str]: Dictionary with cache statistics
    """
    try:
        redis = await get_redis()

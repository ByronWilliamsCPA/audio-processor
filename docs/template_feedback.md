...
### Invalid `$schema` URL in `.claude/settings.json` and `.claude/settings.local.json.example`

- **Priority**: High
- **Category**: Configuration
- **Discovered**: 2026-05-18

**Issue**: The Claude Code settings files generated by the template point `$schema` at `https://json.schemastore.org/claude-code-config.json`, which is not a valid SchemaStore catalog entry. Claude Code rejects the entire file with the error: `Invalid value. Expected one of: "https://json.schemastore.org/claude-code-settings.json"`. When the file is rejected, none of the configured permission rules apply, so every Bash/Read/Write call falls back to the global default and prompts the user.

**Context**: Re-encountered while opening this project in Claude Code. The toast banner reads `Settings file failed to parse: /home/byron/dev/audio_processor/.claude/settings.json. Invalid value. Expected one of: "https://json.schemastore.org/claude-code-settings.json". Permission rules and other settings from this file are not in effect.` This is the second time the issue has surfaced in this project, which means the template is the source.

**Additional finding**: Both files also include an empty `hooks` block whose event keys are camelCase (`preToolUse`, `postToolUse`) instead of the schema-required PascalCase (`PreToolUse`, `PostToolUse`). The block is empty so it is functionally dead, but if a user copies the example and adds hook handlers, those handlers will silently never fire.

**Suggested Fix**:

1. Update the `$schema` URL in both `.claude/settings.json` and `.claude/settings.local.json.example` to `https://json.schemastore.org/claude-code-settings.json`.
2. Remove the empty `hooks: { preToolUse: [], postToolUse: [] }` placeholder from both files. If the template wants to retain a stub for discoverability, use the correct PascalCase event names (`PreToolUse`, `PostToolUse`) and add a comment showing the documented matcher/hooks shape.
3. Consider adding a pre-commit or CI step that validates `.claude/*.json` against the SchemaStore URL so future template drift is caught automatically.

**Affected Files**:

- `{{cookiecutter.project_slug}}/.claude/settings.json`
- `{{cookiecutter.project_slug}}/.claude/settings.local.json.example`
